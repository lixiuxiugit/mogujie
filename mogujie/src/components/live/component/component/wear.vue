...
<transition name="show" >
  <div class="hot" ref="boss">
     <div class="fill"></div>
      <ul>
        <li v-for="(item,index) in lives" :key="index" @click="jion(index)">
          <div class="photo"><span><img :src="item.userphoto" alt=""></span></div>
          <h3>{{item.username}}</h3>
          <img :src="item.banner" alt="" class="banner">
          <div class="imager">
            <div class="now" ref="zhuang">
            ...
            <h1>{{item.title}}</h1>
           <span v-for="(img,index) in item.imgs" :key="index" class="img">
            <img :src="img" alt="">
           </span>
          </div>
        </li>
